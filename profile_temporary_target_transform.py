...
def _do_benchmark(scene: Scene, vector_field, n_trials: int):
...
def main() -> None:
    ...
    for transform_type in ObjectTransformType:
        ...
        cProfile.runctx(
            statement="_do_benchmark(scene, vector_field, 20)",
            globals=globals(),
            locals=locals(),
            filename=f"work/profile_{transform_type.public_name}.prof",
        )
